...
- **只在运行时高亮** — 运行中的速度借用 App 自带的 `text-primary` 类，因此
  回合进行中会突出显示，并自动跟随全部官方主题（高亮是从宿主借用的，本插件
  从不自己定义颜色）。空闲与已完成的读数保持无样式，和状态栏其余部分完全
  一致：不加胶囊、不加边框、不加粗。
...
- 后端按 `(session_id, model)` 聚合 `session_model_usage`——只读最新一行会
  静默丢失其他模型行的 Token。
- 状态栏高亮属于宿主而非本插件：徽标借用 App 自带的 `text-primary` 类，且
  只在回合进行中挂上。插件自己的样式表只有布局（无颜色、字重、边框），因此
  换主题——或 App 将来改变"高亮"的含义——高亮都会自动跟随；复制已解析的
  颜色值只能覆盖前者。
